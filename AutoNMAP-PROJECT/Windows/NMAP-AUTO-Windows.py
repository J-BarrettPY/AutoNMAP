...
def is_ip_responsive(ip):
    try:
        check_cmd = f"powershell nmap -sL -R {ip}"
        response = subprocess.run(check_cmd, shell=True, capture_output=True, text=True, timeout=60)
        if "Host is up" in response.stdout:
            return True
        return False
...
def run_nmap(ip):
    ...
    cmd = f"powershell nmap --script=ftp-anon --script vulners --script-args mincvss=5.0 --script http-grep.nse --script ssl-enum-ciphers -sV -R -p 22,21,23,20,25,53,69,80,110,111,135,137,138,139,143,161,162,443,445,512,513,514,587,993,995,1433,1434,1521,2049,3306,3389,5060,5061,5900,5985,5986,6000,6379,8443,9200,9300,10000,11211,20000,27017,27018 -O -T3 -v {ip}"
...
def upload_to_discord(filename, url, scan_output):
    low_webhook = 'LOW WEBHOOK'
    medium_webhook = 'MEDIUM WEBHOOK'
    high_webhook = 'HIGH WEBHOOK'
    veryhigh_webhook = 'VHIGH WEBHOOK'
    ...
    # freaking bug fixed. no more .txt spam....
    os.remove(filename)


subnet = ip_network('32.0.0.0/8')
...
while True:
    ...
    while len(threads) < max_threads:
        try:
            ip = next(ips_queue)
            thread = threading.Thread(target=run_nmap, args=(ip,))
            thread.start()
            threads.append(thread)
        except StopIteration:
            break

    if not threads and next(ips_queue, None) is None:
        break
